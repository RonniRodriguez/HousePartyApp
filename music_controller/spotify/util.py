...
def is_spotifty_authenticated(session_id):
    tokens = get_user_tokens(session_id)
    if tokens: 
...
def execute_spotify_api_request(session_id, endpoint, post_=False, put_=False):
    ...
    try:
        ...
        if response.status_code == 401:
            # Token expired or unauthorized, refresh token
            refresh_spotify_token(session_id)
            return execute_spotify_api_request(session_id, endpoint, post_, put_)

        if response.status_code == 204:
            return {}  # No content, e.g., no song playing
